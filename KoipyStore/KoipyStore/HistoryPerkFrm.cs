...
{
    public partial class HistoryPerkFrm : TouchForm
    {
        //promotion的DataTable
        ...
        public HistoryPerkFrm(CurrentPerkFrm currentPerkFrm, ProfileLoginFrm profileLoginFrm)
        {
            InitializeComponent();
        ...
        private void HistoryPerkFrm_Load(object sender, EventArgs e)
        {
        ...
        public void Reload()
        {
            ...
            txtInvoice.Text = Program.profileInfo.visit.invoice_no;
            txtConsumption.Text = Program.profileInfo.visit.spending;
            txtCount.Text = Program.profileInfo.visit.people_no;
            ckbExtra.Checked = false;

            this.Text = Program._L("Koipy");
            btnThis.Text = Program._L("This Visit") + "(&T)";
            btnDelete.Text = Program._L("Delete") + "(&D)";
            btnSubmit.Text = Program._L("Submit") + "(&S)";
            lblTitle.Text = Program._L("Last Visit");
            ...
            lblPerk.Text = Program._L("Points") + ":" + Program.profileInfo.total_perk_without_last_visit;
        ...
        {   
            ...
            foreach (Promotion promotion in Program.profileInfo.promotions)
            {
                ...
                m_PromotionTable.Rows.Add(new string[] { promotion.id, promotion.thumbnail, promotion.name, promotion.condition, promotion.redemption_quantity, promotion.perk, "-1", plus_status });
        ...
        private void dataGridViewHistory_CellPainting(object sender, DataGridViewCellPaintingEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                if (e.ColumnIndex == 0)
                {
                    ...
                    using (Brush gridBrush = new SolidBrush(this.dgvHistory.GridColor), backColorBrush = new SolidBrush(e.CellStyle.BackColor))
                    {
        ...
        private void dgvHistory_CellMouseUp(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex == 2)
            {
        ...
        private void updateStatus()
        {
            ...
            double dPerk = Convert.ToDouble(Program.profileInfo.total_perk_without_last_visit) - redemption;       //剩余积分
        ...
        public override void Submit(string param)
        {
            ...
            string url = Program.urlroot + "/store/api/post/?token=" + Program.token
                ...
                + "&profile_id=" + Program.profileInfo.visit.id
                + "&type=" + Program.type
        ...
        private bool check_Request()
        {
            ...
            {
                MessageBox.Show(Program._L("Consumption must not be negative") + "!");
                txtConsumption.Focus();
                return false;
            }
            ...
            {
                MessageBox.Show(Program._L("People number must not be negative") + "!");
                txtCount.Focus();
                return false;
            }
        ...
        {
            string message = Program._L("Confirm to delete") + "?";
            string caption = Program._L("Koipy");
            MessageBoxButtons buttons = MessageBoxButtons.OKCancel;
            MessageBoxIcon icon = MessageBoxIcon.Question;
            if (MessageBox.Show(message, caption, buttons, icon)==DialogResult.OK)
            {
                string url = Program.urlroot + "/store/api/deleteVisit/?token=" + Program.token
                + "&branch_id=" + Program.branch_id
                + "&visit_id=" + Program.profileInfo.visit.id
                + "&language=" + Program.lang;
                string response = KoipyAPIHelper.Instance.getAPI(url);
                if (response == "")
                {
                    Program.log.Error("Cannot call API: " + url);
                    message = Program._L("Cannot connect to Koipy server") + "!";
        ...
        private void HistoryPerkFrm_SizeChanged(object sender, EventArgs e)
        {
        ...
        private void HistoryPerkFrm_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void HistoryPerkFrm_Move(object sender, EventArgs e)
        {
            if (m_currentPerkFrm != null)
                m_currentPerkFrm.Location = Location;
        }
